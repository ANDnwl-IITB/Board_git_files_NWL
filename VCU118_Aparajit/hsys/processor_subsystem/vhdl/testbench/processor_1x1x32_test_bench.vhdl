...
architecture VhpiLink of processor_1x1x32_Test_Bench is -- 
  ...
  signal MAIN_MEM_INVALIDATE_ACK_pipe_read_data : std_logic_vector(0 downto 0);
  signal MAIN_MEM_INVALIDATE_ACK_pipe_read_req  : std_logic_vector(0  downto 0) := (others => '0');
  signal MAIN_MEM_INVALIDATE_ACK_pipe_read_ack  : std_logic_vector(0  downto 0);
...
begin --
  ...
  begin --
    ...
    while true loop -- 
      ...
      MAIN_MEM_INVALIDATE_pipe_write_data <= Unpack_String(val_string,32);
      wait until clk = '1';
  ...
  begin --
    ...
    while true loop -- 
      wait until clk = '0';
      wait for 1 ns; 
      obj_ref := Pack_String_To_Vhpi_String("MAIN_MEM_INVALIDATE_ACK req");
      Vhpi_Get_Port_Value(obj_ref,val_string,1);
      MAIN_MEM_INVALIDATE_ACK_pipe_read_req <= Unpack_String(val_string,1);
      wait until clk = '1';
      obj_ref := Pack_String_To_Vhpi_String("MAIN_MEM_INVALIDATE_ACK ack");
      val_string := Pack_SLV_To_Vhpi_String(MAIN_MEM_INVALIDATE_ACK_pipe_read_ack);
      Vhpi_Set_Port_Value(obj_ref,val_string,1);
      obj_ref := Pack_String_To_Vhpi_String("MAIN_MEM_INVALIDATE_ACK 0");
      val_string := Pack_SLV_To_Vhpi_String(MAIN_MEM_INVALIDATE_ACK_pipe_read_data);
      Vhpi_Set_Port_Value(obj_ref,val_string,1);
      -- 
    end loop;
    --
  ...
  port map ( --
    ...
    MAIN_MEM_INVALIDATE_ACK_pipe_read_data => MAIN_MEM_INVALIDATE_ACK_pipe_read_data,
    MAIN_MEM_INVALIDATE_ACK_pipe_read_req => MAIN_MEM_INVALIDATE_ACK_pipe_read_req,
    MAIN_MEM_INVALIDATE_ACK_pipe_read_ack => MAIN_MEM_INVALIDATE_ACK_pipe_read_ack,
