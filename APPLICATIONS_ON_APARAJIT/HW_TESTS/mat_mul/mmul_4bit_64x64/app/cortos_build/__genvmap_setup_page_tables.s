...
page_table_setup:
   ...
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=43, level=3,  ppnr=0x4002b000, p_addr=0xac, cacheable=0x0, acc=0x3
   ! *(PAGE_TABLE_BASE + 0xac) = 0x4002b0e (pte)
   set 0x4002b0e, %g2
   set 0xac, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
   ...
   add %g5, %g1, %g3
   st %g2, [%g3]
   !PTE: context=0, index=48, level=3,  ppnr=0x40030000, p_addr=0xc0, cacheable=0x1, acc=0x4
   ! *(PAGE_TABLE_BASE + 0xc0) = 0x4003092 (pte)
   set 0x4003092, %g2
   set 0xc0, %g5
   add %g5, %g1, %g3
   st %g2, [%g3]
