...
K_SIZE = 64   # Cols of A, Rows of B (The Deep Inner Dimension)
N_SIZE = 32   # Cols of B, Cols of C

# Base byte addresses
# A takes 1024 bytes (32 * 64 * 0.5)
A_BASE = 64
# B takes 1024 bytes (64 * 32 * 0.5)
B_BASE = A_BASE + 1024    # 1088
# C takes 8192 bytes (32 * 32 * 8 bytes per unpacked element)
C_BASE = B_BASE + 1024    # 2112
...
def main():
    ...
    print("Swizzling Matrix A (32x64 M-Tiles)...")
    for row in range(M_SIZE):
        for col in range(K_SIZE):
            # A_Base + (RowGrp * 256) + (SubGrp * 128) + (K_Chunk * 32) + (RowInSub * 8)
    ...
    print("Swizzling Matrix B (64x32 N-Tiles)...")
    for row in range(K_SIZE):
        for col in range(N_SIZE):
            # B_Base + (ColGrp * 256) + (SubGrp * 128) + (K_Chunk * 32) + (ColInSub * 8)
    ...
    # Write Output Files with APARA Hardware Offset
    ...
    print(f"Success! APARA-ready unpacked (32x64 * 64x32) files generated.")
    print(f"Hardware Memory Map (Word Indices): A_Base=0x{APARA_OFFSET + A_BASE//8:X}, B_Base=0x{APARA_OFFSET + B_BASE//8:X}, C_Base=0x{APARA_OFFSET + C_BASE//8:X}")
