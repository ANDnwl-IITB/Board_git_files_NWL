...
#define NBYTES         (24*512*4)   // 24*FFT_SIZE*MAX_NTHREADS
...
complex64 fft_data[FFT_SIZE];
complex64 twiddle[FFT_SIZE/2];
...
static inline complex64 cmul(complex64 a, complex64 b)
{
    complex64 c;
...
void init_fft_data()
{
    ...
    {
        double angle = -2.0 * PI * ((double)i) / FFT_SIZE;

        twiddle[i].real = cos(angle);
        twiddle[i].imag = sin(angle);
    }
}

/* Use all 4 only if you are using 4 threads , if you are using only 1 thread pls comment out the rest 3.*/

void set_args ()   
{
	while(writeThreadDataRegister (&hms, 0, 0, 0, 17, ENGINE_DMEM_BASE_POINTER));
	while(writeThreadDataRegister (&hms, 0, 0, 1, 17, ENGINE_DMEM_BASE_POINTER + 3*8*512));     // 3*8*FFT_SIZE
	while(writeThreadDataRegister (&hms, 0, 0, 2, 17, ENGINE_DMEM_BASE_POINTER + 3*8*512*2));   // 3*8*FFT_SIZE*2
	while(writeThreadDataRegister (&hms, 0, 0, 3, 17, ENGINE_DMEM_BASE_POINTER + 3*8*512*3));   // 3*8*FFT_SIZE*3
}
...
void fft512_on_processor()
{
    ...
    /* Local copy of twiddle factors */
    complex64 twiddle_stage[FFT_SIZE/2];

    for(i = 0; i < FFT_SIZE/2; i++)
        twiddle_stage[i] = twiddle[i];

    uint32_t num_twiddles = FFT_SIZE/2;

    uint64_t t0 = __ajit_get_clock_time();

    for(step = FFT_SIZE; step >= 2; step >>= 1)
    {
        half_step = step >> 1;

        for(group = 0; group < FFT_SIZE; group += step)
        {
            for(j = 0; j < half_step; j++)
            {
                complex64 a = fft_data[group + j];
                complex64 b = fft_data[group + j + half_step];

                complex64 diff;
                complex64 temp;

                /* Upper butterfly output */
                fft_data[group + j].real = a.real + b.real;
                fft_data[group + j].imag = a.imag + b.imag;

                /* Lower butterfly input */
                diff.real = a.real - b.real;
                diff.imag = a.imag - b.imag;

                /* Twiddle multiplication */
                temp = cmul(diff, twiddle_stage[j]);

                fft_data[group + j + half_step] = temp;
            }
        }

        /*------------------------------------------
         * Generate twiddles for the next stage
         * W -> W^2
         *-----------------------------------------*/

        if(step > 2)
        {
            for(i = 0; i < num_twiddles/2; i++)
            {
                twiddle_stage[i] =
                    cmul(twiddle_stage[2*i],
                         twiddle_stage[2*i]);
            }
    ...
    cortos_printf("Processor FFT completed.\n");
    cortos_printf("Processor ticks = %lld\n", processor_ticks);
}
...
void main_00 () 
{
	...
	cortos_printf("calling fft\n");
	//dotp_on_processor();
	fft512_on_processor();
	fft_across_all_threads();
	cortos_printf("done with fft\n");
	
	uint64_t *ptr0 = (uint64_t *) ENGINE_DMEM_BASE_POINTER;
...
/*	
	double* ptr0 = (float*) ENGINE_DMEM_BASE_POINTER;
	for(I = 0; I < NBYTES/8; I++)
	{
		float rv = ptr0[I];	
		cortos_printf ("ptr0[%d] error %x\n", I, rv);
	}
