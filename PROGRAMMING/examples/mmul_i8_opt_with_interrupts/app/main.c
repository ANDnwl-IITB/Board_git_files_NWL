...
volatile int8_t volatile A[ORDER][ORDER] __attribute__((aligned(64)));
volatile int8_t volatile B[ORDER][ORDER] __attribute__((aligned(64)));

volatile int8_t volatile TRESULT[ORDER][ORDER] __attribute__((aligned(64)));
volatile int8_t volatile PRESULT[ORDER][ORDER] __attribute__((aligned(64)));
...
volatile uint32_t iflags[NTHREADS];
				...
				uint32_t element_size_in_bytes)
{
	...
	for(TID = 0; TID < NTHREADS; TID++)
	{
		while(runEngineThreadAndInterrupt (&hms, 0,0,TID,0x0));
	}

	uint64_t t1 = __ajit_get_clock_time ();
	setup_ticks = (t1 - t0);

	// cortos_printf ("Info: finished mmul_start_across_all_threads\n");
}

//
// poll the interrupt flags and wait until they have been set
...
void mmul_complete_across_threads (uint32_t RSTRIPE_ID, uint32_t CSTRIPE_ID)
{
	...
	while(1)
	{
		if(iflags[0] && iflags[1] && iflags[2] && iflags[3])
		{
			iflags[0] = 0;
			iflags[1] = 0;
			iflags[2] = 0;
				 ...
				 uint32_t bytes_per_row)
{
	...
	// cortos_printf("Info: load_B_stripe completed\n");
}
				 ...
				 uint32_t bytes_per_row)
{
	...
	// cortos_printf("Info: load_A_stripe completed\n");
}
...
void mmul_simple_check()
{
	...
	for(TID = 0; TID < NTHREADS; TID++)
	{
		...
		while(checkIfThreadHasCompleted (&hms, 0,0,TID, exc, spc));
	}
							...
							uint32_t bytes_per_row)
{
	...
	for (S = 0; S < nstripes; S++)
	{
		...
		for (T = 0; T < nstripes; T++)
		{
			...
			uint64_t tbegin = __ajit_get_clock_time ();
...
int check_result_values()
{
	...
	{
		int8_t refV = PRESULT[0][0];
		int8_t V    = TRESULT[0][0];
	...
	if(ret_val == 0)
	{
		...
		cortos_printf ("Net speedup = %F, computation speedup = %F\n",
				((double) processor_ticks)/engine_ticks, ((double) processor_ticks)/computation_ticks);
	}
	else
...
void main_00 () 
{
	...
	cortos_printf("enabling thread daemons and interrupts,  and mmu channel.\n");
	for(TID = 0; TID < NTHREADS; TID++)
	{
		...
		enableThreadInterrupt (&hms, 0, 0, TID);
		uint32_t cr = readFromEngineConfigRegister(&hms, 0,0,TID,0);
		cortos_printf("Info: after enabling thread %d, cr=0x%x\n", TID, cr);
	}
	...
	//
	cortos_printf ("Info: enable AJIT interrupt 14\n");
	enableInterrupt (0,0,14);
...
{
	// disable interrupt..
	disableInterrupt(0,0,14);

	//__ajit_serial_puts__ ("->ISR\n", 8);

	//
	// check if DMA interrupt in one of the channels is 
	// up.
	//
	int I;
	for(I = 0; I < NTHREADS; I++)
	{
		if(threadInterruptIsSet (&hms, 0, 0, I))
		{
			// cortos_printf ("Interrupt for thread %d is set\n", I);

			iflags[I] = 1;

			clearThreadInterrupt (&hms, 0, 0, I);
			releaseEngineThread (&hms, 0,0, I);
		}
	}
