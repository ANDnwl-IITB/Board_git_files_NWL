...
.L52:
	...
	.align 8
.LC4:
	.asciz	"Entered mmul_complete_across_threads\n"
	.section	".text"
	.align 4
...
mmul_complete_across_threads:
	save	%sp, -96, %sp
	sethi	%hi(.LC4), %o0
	sethi	%hi(exceptions), %i2
	or	%o0, %lo(.LC4), %o0
	sethi	%hi(stop_pcs), %i3
	call	cortos_printf, 0
	 sethi	%hi(hms), %i4
	mov	0, %i5
	or	%i2, %lo(exceptions), %i2
	or	%i3, %lo(stop_pcs), %i3
	or	%i4, %lo(hms), %i4
	sll	%i5, 2, %o5
.L56:
	mov	%i5, %o3
...
dma_insert_R_into_result:
	...
	.align 4
	.global mmul_simple_check
	.type	mmul_simple_check, #function
	.proc	020
mmul_simple_check:
	save	%sp, -184, %sp
	call	__ajit_get_clock_time, 0
	...
	call	dma_load_stripe, 0
	 or	%o1, %lo(B), %o1
	call	__ajit_get_clock_time, 0
	 sethi	%hi(16777216), %l5
	mov	%o0, %l0
	mov	%o1, %l1
	mov	0, %l2
	sethi	%hi(16793600), %l7
	sethi	%hi(16826368), %l6
	mov	0, %i0
	mov	127, %i1
	mov	0, %i2
	mov	4095, %i3
...
.L95:
	...
	addcc	%l1, %i5, %g3
	addx	%l0, %i4, %g2
	std	%g2, [%g1+%lo(engine_ticks)]
	jmp	%i7+8
	 restore
	.size	mmul_simple_check, .-mmul_simple_check
	.section	.rodata.str1.8
	.align 8
.LC6:
	...
	.align 8
.LC7:
	.asciz	"finished mmul_stripe_and_run_on_threads (COUNT=%d, engine-ticks=%lld)\n"
	.section	".text"
	.align 4
...
.L111:
	sethi	%hi(dma_ticks), %g1
	sethi	%hi(setup_ticks), %i1
	ldd	[%g1+%lo(dma_ticks)], %o4
	sethi	%hi(computation_ticks), %i0
	ldd	[%i1+%lo(setup_ticks)], %i2
	sethi	%hi(engine_ticks), %l1
	ldd	[%i0+%lo(computation_ticks)], %g2
	ldd	[%l1+%lo(engine_ticks)], %i4
	std	%i4, [%fp-8]
	ld	[%fp-4], %g1
	ld	[%fp-8], %g4
	...
	call	__floatundidf, 0
	 std	%f10, [%fp-24]
	ldd	[%i0+%lo(computation_ticks)], %o0
	fmovs	%f0, %f8
	ldd	[%i1+%lo(setup_ticks)], %g2
	fmovs	%f1, %f9
	subcc	%o1, %g3, %o1
	std	%f8, [%fp-16]
	call	__floatundidf, 0
