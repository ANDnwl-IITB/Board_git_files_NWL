...
void set_args(int64_t a, int64_t b)
{
	while(writeThreadDataRegister (&hms, 0, 0, 0,1, a));
	while(writeThreadDataRegister (&hms, 0, 0, 0,2, b));
}

int print_status = 1;
int  check_ops (int64_t a, int64_t b)
{
	...
	while(checkIfThreadHasCompleted (&hms, 0,0,0, &exceptions, &bundle_word_index));
	...
	if(ev != rval)
	{
		cortos_printf("Error: %lld * %lld =  %lld, expected %lld\n", a,b,rval,ev);
		return(1);
	}
...
void main_00 () 
{
	...
	int64_t a, b;
	int status = 0;
	for (a = 1; a < (1 << 62); a = 2*a)
	{
		for (b = 1; b < (1 << 62); b = 2*b)
		{
