...
void set_args(int32_t a, int32_t b)
{
	while(writeThreadDataRegister (&hms, 0, 0, 0,1,(int64_t)  a));
	while(writeThreadDataRegister (&hms, 0, 0, 0,2,(int64_t)  b));
}


int  check_ops (int32_t a, int32_t b)
{
	...
	while(runEngineThread (&hms, 0,0,0,0x0));
	while(completeEngineThread (&hms, 0,0,0, &exceptions, &bundle_word_index));

	int64_t rval;
	while(readThreadDataRegister(&hms, 0,0,0,3, &rval));
	int32_t rval32 = (rval & 0xffffffff);
	...
	if(ev != rval32)
	{
		cortos_printf("Error: %d * %d =  %d, expected %d\n", a,b,rval32,ev);
		return(1);
	}

	while(readThreadDataRegister(&hms, 0,0,0,4, &rval));
	rval32 = (rval & 0xffffffff);
	...
	if(b != 0)
	{
		...
		if(ev != rval32)
		{
			cortos_printf("Error: %ld / %d =  %d, expected %d\n", a,b,rval32,ev);
			return(1);
		}
	...
	if ((b >= 0) && (b < 32))
	{
		while(readThreadDataRegister(&hms, 0,0,0,7, &rval));
		rval32 = (rval & 0xffffffff);
		...
		// right-shift
		while(readThreadDataRegister(&hms, 0,0,0,8, &rval));
		rval32 = (rval & 0xffffffff);
...
void main_00 () 
{
	...
	int64_t a, b;
	int status = 0;
	for (a = 1; a < (1 << 30); a = 2*a)
	{
		for (b = 1; b < (1 << 30); b = 2*b)
		{
