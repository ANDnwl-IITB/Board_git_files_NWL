...
int main()
{
	...
	while(1)
	{
		...
		// Extract pointers to the destination and source MAC addresses
    		...
    		uint8_t* eth_type = (uint8_t*)bufptrVA + 12 + 24;
   		uint8_t* data = (uint8_t*)bufptrVA + 14 + 24;
		...
		/*if(eth_type[0] == 136 && eth_type[1] == 181)
		{
				for(i=0; i< ((packetLen-30)/2); i++)
				{
					temp=data[i];
					data[i]=data[packetLen-31-i];
					data[packetLen-31-i]= temp;
				}
		}*/
